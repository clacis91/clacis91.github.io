...
크게 *List*, *Set*, *Map* 이렇게 3개의 인터페이스로 나눌 수 있다. 각각은 Collection 인터페이스를 상속하고 있다.

* List : 순서를 갖고 있지만 중복이 허용된 객체들의 집합
* Set : 중복을 허용하지 않는 객체들의 집합
* Map : Key-Value 쌍으로 이루어졌으며 고유값인 Key와 연결된 객체들을 관리하는 집합
...
위 클래스들은 어차피 검색하면 메소드나 사용법은 다 나온다. 결국 어느 상황에 어떤 클래스를 선택하느냐가 중요.  
고려할 것은 *속도*와 *Thread-safe*다. 속도가 빠른 클래스는 동기화 문제가 있고, 이것을 해결하면 bottle neck이 발생한다. 이러한 trade-off를 잘 고려해서 사용해야 한다.

| 인터페이스 | 가장 빠른 클래스 | Thread-safe 클래스 |
|:--------:|:--------:|:--------:|
...
둘다 Collection 반복 처리를 위한 객체이다. 차이는?  
Enumeration은 thread-safe한 클래스에서 제공하며 Iterator는 그렇지 않은 클래스에서 제공한다.  
